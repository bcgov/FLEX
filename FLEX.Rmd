...
'While classic population-level modeling is limited in its ability to integrate decision-making in its actual complexity, the development of individual- or agent-based models(IBM/ABMs) has opened the possibility of describing the way that decisions are made, and their effects, in minute detail' (DeAngelis & Diaz, 2019)

FLEX is a decision-support tool to map potential consequences of management decisions on fisher populations. The FLEX engine is an individual-based model (IBM), written in the framework of SpaDES (spatially explicit discrete event simulations; https://predictiveecology.org/SpaDES/). FLEX uses empirical data on survival, reproduction and habitat relationships to forecast a spatial population response to changes in the landscape. FLEX integrates with other SpaDES modules (https://github.com/bcgov/clus/tree/main/R/SpaDES-modules), which provide projected habitat layers annually.
...
![Conceputal diagram of FLEX (inputs, events, and outputs) and integration with CLUS.](/figures/FLEX_SpaDES_conceptual.png)
...
flexRasWorld is required (https://github.com/bcgov/clus/tree/main/R/SpaDES-modules)
